...
- (instancetype)initWithPreView:(UIView*)preView ArrayObjectType:(NSArray*)arrayBarCodeType cropRect:(CGRect)cropRect
              success:(void(^)(NSArray *array))blockScanResult
{
    if (self = [super init])
    {
        ...
        {
            _scanNativeObj = [[LBXScanNative alloc]initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray *array) {
 ...
 */
- (instancetype)initZXingWithPreView:(UIView *)preView success:(void(^)(NSArray *array))blockScanResult
{
    if (self = [super init])
    {
 ...
 *  @param block 返回识别结果
 */
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray *array))block;
{    
...
+ (NSString*)convertZXBarcodeFormat:(ZXBarcodeFormat)barCodeFormat
{
    ...
    switch (barCodeFormat) {
        ...
        case kBarcodeFormatCode128:
            ...
            break;
        case kBarcodeFormatDataMatrix:// 改过
